...
//             curr_tick = (int)totalTicksDouble - 1;
...
//                         Debug.Log($"[{Time.time:F2}] WINDOW OPENING at tick {curr_tick} (measure {curr_meas}) - Input window: {inputWindowTicksBefore} tick(s) before + {inputWindowTicks} tick(s) after, Note duration: {current_note_duration} sixteenths");
...
//         double ticksInPeriod = timeSinceLastChange * (bpmToUse / 60.0) * 4.0;
//         accumulatedTicks += ticksInPeriod;
        
//         Debug.Log($"[BPM CHANGE] At {bpmChanges[currentBPMChangeIndex].timeInSeconds}s: " +
//                   $"Added {ticksInPeriod:F2} ticks from previous period (BPM {bpmToUse}). " +
...
//         // NEW: Adjust input windows when BPM reaches 170
//         if (bpm >= 170)
//         {
//             inputWindowTicks = 1;
